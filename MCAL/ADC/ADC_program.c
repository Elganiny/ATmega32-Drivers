...
void ADC_init(const ADC_ConfigType * Config_Ptr)
{
	...
	else/* no action is taken */
	{
...
#else
		/* For turning on the "ADC left adjust result" ADLAR bit is set by 1 */
		ADMUX = (1<<ADMUX_ADLAR) | (1 << ADMUX_REFS1);
#endif
	}
		...
		ADCSRA = Config_Ptr -> prescaler | (1<<ADCSRA_ADEN) | (1<<ADCSRA_ADIE);
	#endif
		...
		ADCSRA = Config_Ptr -> prescaler | (1<<ADCSRA_ADEN) | (1<<ADCSRA_ADATE);
	#else
		/* ADIE is set here for enabling ADC interrupt */
		ADCSRA = Config_Ptr -> prescaler | (1<<ADCSRA_ADEN) | (1<<ADCSRA_ADATE) | (1<<ADCSRA_ADIE);
	#endif
